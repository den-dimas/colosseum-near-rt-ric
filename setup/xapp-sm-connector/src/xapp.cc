...
std::string Xapp::get_kpm_data() {
    mdclog_write(MDCLOG_INFO, "Attempting to retrieve KPM data from SDL...");

    // In a real implementation, you would check for a successful SDL response.
    bool sdl_data_available = false;

    rapidjson::Document kpm_data;
    kpm_data.SetObject();

    if (sdl_data_available) {
        kpm_data.AddMember("cell_load", 0.75, kpm_data.GetAllocator());
        kpm_data.AddMember("packet_drop_rate", 0.05, kpm_data.GetAllocator());
    }
    else {
        mdclog_write(MDCLOG_INFO, "SDL data not available. Using dummy data.");
        // Dummy KPM data
        kpm_data.AddMember("cell_load", 0.85, kpm_data.GetAllocator());
        kpm_data.AddMember("packet_drop_rate", 0.02, kpm_data.GetAllocator());
    }

    rapidjson::StringBuffer buffer;
    rapidjson::Writer<rapidjson::StringBuffer> writer(buffer);
    kpm_data.Accept(writer);
...
void Xapp::handle_rx_msg_agent(std::string agent_ip) {
    ...
    if (it != agentIp_socket.end()) {
        ...
        // The logic loop for the client socket (xapp-sm-connector)
        while (true) {
            std::string kpm_data_json = get_kpm_data();

            int sent_bytes = send(control_sckfd, kpm_data_json.c_str(), kpm_data_json.length(), 0);
            if (sent_bytes == -1) {
                mdclog_write(MDCLOG_ERR, "Error sending KPM data to Python xApp.");
                break;
            }
            mdclog_write(MDCLOG_INFO, "Sent KPM data to Python xApp.");

            int rcv_size = recv(control_sckfd, buf, max_size, 0);
            if (rcv_size > 0) {
                std::cout << "Received message from Python xApp: " << buf << std::endl;

                rapidjson::Document control_params;
                control_params.Parse(buf);

                if (control_params.HasParseError()) {
                    mdclog_write(MDCLOG_WARN, "Received invalid JSON from Python xApp. Ignoring.");
                }
                else if (control_params.HasMember("bandwidth_adjustment") && control_params["bandwidth_adjustment"].IsInt()) {
                    int bandwidth_adjustment_value = control_params["bandwidth_adjustment"].GetInt();
                    std::string target_gnb = "";

                    if (control_params.HasMember("target_gnb") && control_params["target_gnb"].IsString()) {
                        target_gnb = control_params["target_gnb"].GetString();
                    }
                    else {
                        mdclog_write(MDCLOG_WARN, "Target gNB not specified in JSON, using default gNB.");
                        std::map<std::string, std::string>::iterator it_gnb;
                        it_gnb = agentIp_gnbId.find(agent_ip);
                        if (it_gnb != agentIp_gnbId.end()) {
                            target_gnb = it_gnb->second;
                        }
                    }

                    // Convert the integer value to a string to pass to the existing send_ric_control_request
                    std::string payload_str = std::to_string(bandwidth_adjustment_value);
                    char* payload_cstr = const_cast<char*>(payload_str.c_str());

                    if (!target_gnb.empty()) {
                        send_ric_control_request(payload_cstr, target_gnb);
                    }
                    else {
                        mdclog_write(MDCLOG_WARN, "ERROR: No gNB ID found for agent %s", agent_ip.c_str());
                    }
                }
                else {
                    mdclog_write(MDCLOG_WARN, "Received unknown message format from Python xApp.");
                }

                memset(buf, 0, max_size);
            }
            else {
                mdclog_write(MDCLOG_WARN, "No control data received from Python xApp. Retrying.");
            }

            // Wait for a period before the next control cycle
            sleep(5);
        }
    }
}
